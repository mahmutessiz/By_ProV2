...
namespace By_ProV2.DataAccess
{
    public class SutRaporRepository
    {
        ...
        /// <summary>
        /// Seçilen tarihe göre günlük süt satış kayıtlarını getirir (Depodan Sevk ve Direkt Sevk).
        /// </summary>
        public List<SutKaydi> GetGunlukSutSatisKayit(DateTime tarih)
        {
            List<SutKaydi> kayitlar = new List<SutKaydi>();
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                string query = @"
                    ...
                    LEFT JOIN Cari m ON s.MusteriId = m.CariId
                    WHERE CAST(s.Tarih AS DATE) = @Tarih
                    AND (s.IslemTuru = 'Depodan Sevk' OR s.IslemTuru = 'Direkt Sevk')";
