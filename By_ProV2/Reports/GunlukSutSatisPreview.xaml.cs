...
{
    public partial class GunlukSutSatisPreview : Window
    {
        private readonly SutRaporRepository _sutRepo = new SutRaporRepository();

        public GunlukSutSatisPreview()
        {
            InitializeComponent();
        }
        ...
        private void btnGetir_Click(object sender, RoutedEventArgs e)
        {
            ...
            var sutKayitlar = _sutRepo.GetGunlukSutSatisKayit(secilenTarih);
            ...
            // Başlık
            Paragraph header = new Paragraph(new Bold(new Run($"GÜNLÜK SÜT SATIŞ RAPORU - {secilenTarih:dd.MM.yyyy}")))
            {
            ...
            // 🚚 Depodan Sevk Yapılan Sütler (Satış)
            var depodanSevk = sutKayitlar.Where(x => x.IslemTuru == "Depodan Sevk").ToList();
            if (depodanSevk.Any())
            {
                doc.Blocks.Add(CreateSectionTitle("Depodan Sevk Edilen (Satış) Sütler"));
                doc.Blocks.Add(CreateTable(depodanSevk));
            }

            // 🚛 Direkt Sevk Yapılan Sütler (Satış)
            ...
            if (direktSevk.Any())
            {
                ...
                doc.Blocks.Add(CreateSectionTitle("Direkt Sevk Yapılan (Satış) Sütler"));
        ...
        private Table CreateTable(List<SutKaydi> kayitlar)
        {
            ...
            table.Columns.Add(col1); // Müşteri
            ...
            string[] headers = { "Müşteri", "Miktar (lt)", "Net Miktar (lt)", "Yağ (%)", "Protein (%)", "TKM (%)", "Laktoz (%)", "pH ", "İletkenlik (mS)", "Donma Noktası (°C)", "Kesinti (lt)", "Antibiyotik", "Durum", "Açıklama" };
        ...
        private void btnYazdir_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (printDialog.ShowDialog() == true)
                {
                    ...
                    printDialog.PrintDocument(idpSource.DocumentPaginator, "Günlük Süt Satış Raporu (Yatay)");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnPDF_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                FileName = $"GunlukSutSatisRaporu_{secilenTarih:yyyyMMdd}.pdf",
                DefaultExt = ".pdf",
            ...
            if (dlg.ShowDialog() == true)
            {
                try
                {
                    ...
                    document.Info.Title = "Günlük Süt Satış Raporu";
                    document.Info.Author = "By_ProV2";
                    document.Info.Subject = $"Günlük Süt Satış Raporu - {secilenTarih:dd.MM.yyyy}";
                    ...
                    // Başlık
                    string baslik = $"GÜNLÜK SÜT SATIŞ RAPORU - {secilenTarih:dd.MM.yyyy}";
                    ...
                    if (depodanSevk.Any())
                    {
                        gfx.DrawString("Depodan Sevk Edilen (Satış) Sütler", altBaslikFont, XBrushes.DarkSlateGray, new XRect(0, yPos, page.Width, 20), XStringFormats.Center);
                        yPos += 20;
                
                        yPos = CreatePdfTableWithPageBreakSupport(gfx, page, document, depodanSevk, yPos, normalFont, kalinFont);
                    }
                    ...
                    if (direktSevk.Any())
                    {
                        ...
                        gfx.DrawString("Direkt Sevk Yapılan (Satış) Sütler", altBaslikFont, XBrushes.DarkSlateGray, new XRect(0, yPos, page.Width, 20), XStringFormats.Center);
                        yPos += 20;
        ...
        private double CreatePdfTableWithPageBreakSupport(XGraphics gfx, PdfPage page, PdfDocument document, List<SutKaydi> kayitlar, double yPos, XFont normalFont, XFont kalinFont)
        {
            ...
            string[] headers = { "Müşteri", "Miktar", "Net Miktar", "Yağ", "Prot", "TKM", "Laktoz", "pH", "İletkenlik", "Donma", "Kesinti", "Anti", "Durum", "Açıklama" };
